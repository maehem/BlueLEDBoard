...
void refreshRow() {
  ...
  while (spiCount--) {
      SPDR = spiBuffer[--spiBufferPtr];       // Start transmitting the next SPI byte (pre-decrement
  ...
  while (miniBufferReaderPtr<miniBufferPtr) {
      // we still must keep manually checking the serial buffer becuase unloading
      // the mini buffer tkaes longer than incoming serial data to fill the buffer.

      if ( UCSR0A & _BV( RXC0 ) ) {   // If a char is available, save it in the
                                      // minibuffer. We will natually get to it
                                      // at the end of this copy.
        unsigned char c = UDR0;
        *(miniBufferPtr++) = c;
      }
  ...
  if ( isr_row == 0 ) {     // Did we scan out all the rows?
    ...
    isr_row = ROWS;
    sync=0;                         // Signal vertical retrace so forground knows when
                                    // to start drawing to avoid tearing on the display.

    if (packetFlag) {     // New packet available in serial buffer?

      // TODO: Just update a pointer rather than mempying the whole thing over

      memcpy( spiBuffer , readBuffer , BUFFER_SIZE );   // Copy the entire buffer even though
                                                        // it might not be full just to keep timing consistant
      packetFlag =0;
