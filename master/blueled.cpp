...
// a font is an array of 256 chartypes, indexed by ascii.
// A null cols[] indicates the char is undefined
...
void dumpFont( chartype *font) {
	...
	for( int b =0; b <256; b++ ) {
		if (font[b].cols != NULL) {				// defined char?
			...
			// Exploits the fact that blank cols
			// have no bits set so are arethmically 0.
			// we do left < right rather then <= so that we
			// are always left with at least 1 blank col
			while (left<right && font[b].cols[left]==0 ) {
				left++;
			}

			// Trim blank cols from the left
			// Exploits the fact that blank cols have no
			// bits set so are arethmically 0.
...
int main(int argc, char **argv) {
    ...
    while (1) {
		...
		// On the first frame of each pass, the 1st col of the message will
		// be in the last col of the display.
		// On the last frame of each pass, the last col of the message will
		// be in the last col of the display.
		// This way there is a seamless transision from pass to pass
		// We also repeast the message to fill in the gap if the message is
		// shorter than the display.
		// We also keep the previous message to scroll that off and to have
		// a seamless transision between messages.

		// First we transision from the previous message to the new none
		// we always end with the last message col of the message on the
		// last col of the display, so pick up from there using the previous message
		...
		// Ok, now we are past the end of the previous message,
		// now scroll out just the new message.
